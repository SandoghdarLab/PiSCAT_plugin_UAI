...
def dnn_anomaly_protein_analysis(paths, video_names, hyperparameters, flags, name_mkdir):
    ...
    for idx_vid, (p_, n_) in enumerate(zip(paths, video_names)):
        ...
        if isinstance(hyperparameters['start_fr'], int) and isinstance(hyperparameters['end_fr'], int):
            video_ = reading_videos.video_reader(file_name=p_ + '/' + n_,
                                                 type=hyperparameters['img_reader_type'],
        ...
        if flags['DNN_train']:
            if idx_vid == 0 and flags['Train_from_random_weights']:
            ...
            dnn_ = FastDVDNet(video_original=video_norm, gpu_device_num=hyperparameters['gpu_device_num'])
